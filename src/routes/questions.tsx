...
import { type Question, type QuestionAnswer, questions as orderedQuestions } from "../data";
  ...
  return (
    <div className="h-full flex flex-col gap-4">
      ...
      <nav className="w-full grid grid-cols-2 gap-2 py-10">
        <button type="button" className="p-4" onClick={prevQuestion}>
          ‹ Zurück
        </button>
        {(() => {
          if (complete)
            return (
              <Link to="/result" search={{ r: getResultFromAnswers(answers) }} className="p-4 font-bold text-center">
                zur Auswertung ›
              </Link>
            );

          return (
            <button type="button" className="p-4 text-center" onClick={nextQuestion}>
              Weiter ›
            </button>
          );
        })()}
